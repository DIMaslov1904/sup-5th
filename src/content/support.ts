...
const modal = createModal("iframe-modal", prevOnOpenModal, nextOnCloseModal) as ExtendedModal;
...
const getBtns = async () => { 
  ...
  d.querySelectorAll('.ticket-toolbar-bg .buttons').forEach(wrap => { 
    ...
    b.addEventListener('mouseover', () => { b.classList.add('ui-state-hover') })
    b.addEventListener('mouseout', () => { b.classList.remove('ui-state-hover') })
...
const startObserverTicketWrapper = async () => { 
  const wrap = (await waitForElm('#ticket-wrapper')).parentElement as HTMLDivElement;
  getBtns();
