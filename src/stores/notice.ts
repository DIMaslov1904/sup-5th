...
export const useNoticeStore = defineStore(STORAGE_NAME, () => {
  ...
  const add = (type: NoticeType, text: string, duration: number = 86400) => {
    state.value = [...state.value, { timestamp: new Date().getTime(), type, text, timestampEnd: new Date().getTime() + duration * 1000}]
    saveToStorage()
  }

  const remove = (timestamp: number) => {
    state.value = state.value.filter((item: Notice) => item.timestamp !== timestamp)
    saveToStorage()
  }
